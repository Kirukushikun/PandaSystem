...
@elseif($mode === "view")
    <form class="h-full {{$mode == 'create' ? 'pb-8' : ''}}" wire:submit.prevent="submitForm">
        <div class="form-buttons absolute top-[35px] right-[40px] flex gap-3 justify-end">
            <div class="request-status bg-yellow-100 text-yellow-500">{{$request->request_status}}</div>
        ...
        >
            <div class="input-fields grid grid-cols md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="input-group">
                    <label for="employee_name">Employee Name:</label>
                    <input type="text" name="employee_name" id="employee_name" x-ref="employee_name" wire:model="employee_name" value="{{$request->employee_name ?? ''}}">
                </div>
                <div class="input-group">
                    <label for="employee_id">Employee ID:</label>
                    <input type="text" name="employee_id" id="employee_id" x-ref="employee_id" wire:model="employee_id" value="{{$request->employee_id ?? ''}}">
                </div>
                <div class="input-group">
                    <label for="department">Department:</label>
                    <select name="department" id="department" x-ref="department" x-ref="department" wire:model="department" >
                        <option value="">Select department</option>
                        @php
                            $departments = ['FEEDMILL', 'FOC', 'GENERAL SERVICES', 'HR','IT & SECURITY', 'POULTRY', 'PURCHASING', 'SALES & MARKETING', 'SWINE']
                        @endphp
                        @foreach($departments as $department)
                            <option value="{{$department}}" {{$department == $request->department ? 'Selected' : ''}}>{{$department}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="input-group">
                    ...
                    <select name="type_of_action" id="type_of_action" x-ref="type_of_action" wire:model="type_of_action">
                        <option value="">Select type</option>
                        @php
                            $actionTypes = ['Regularization', 'Salary Alignment', 'Wage Order', 'Lateral Transfer','Developmental Assignment', 'Interim Allowance', 'Promotion', 'Training Status', 'Confirmation of Appointment']
                        @endphp
                        @foreach($actionTypes as $action)
                            <option value="{{$action}}" {{$action == $request->type_of_action ? 'Selected' : ''}}>{{$action}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="input-group h-full">
                <label for="justification">Justification:</label>
                <textarea name="justification" id="justification" class="w-full h-full resize-none" x-ref="justification" wire:model="justification">
            ...
            <!-- Buttons -->
            <div disabled:="!showAction" x-show="showAction" class="form-buttons  bottom-0 right-0 flex gap-3 justify-end md:pb-0 md:mb-0 md:absolute">
                <button type="button" @click="validateBeforeModal('Confirm Resubmission', 'Are you sure you want to resubmit this request for processing?'); formAction = 'resubmitRequest' " class="border border-3 border-gray-600 bg-gray-600 text-white hover:bg-gray-800 px-4 py-2">Resubmit to HR</button>
